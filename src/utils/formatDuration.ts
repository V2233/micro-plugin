...
/**
 * 补零函数。
 * @param {number} num - 要计算的时间（以秒为单位）。
 * @param {boolean} [repair] - 修复小时、分钟和秒的显示格式的可选参数。如果设置为true，并且小时、分钟或秒小于10，则在值前面添加零。
 * @returns {string} - 包含天、小时、分钟和秒的时间对象。
 */
function padWithZero(num:number, repair:boolean) {  
 ...
 */
function computeTimeObject(time: number, repair = true) {  
